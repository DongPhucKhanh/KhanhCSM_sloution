...
const BlogCategoryList = ({ selectedBlogCategoryId, onSelectBlogCategory }) => {
    ...
    if (loading) {
        return <div className="text-center my-3 text-muted small"><i className="fas fa-spinner fa-spin mr-2"></i>Ðang n?p ch? ð?...</div>;
    }

    return (
        <div className="modern-card">
            <div className="list-group list-group-flush">
                {/* Nút xem T?t c? bài vi?t */}
                <button
                    type="button"
                ...
                >
                    <span><i className="fa-solid fa-list-check mr-2 text-muted"></i>T?t c? bài vi?t</span>
                    <span className="badge badge-light border text-muted px-2 py-1" style={{ fontSize: '0.7rem' }}>All</span>
                </button>

                {blogCategories.length === 0 ? (
                    <div className="py-2 text-center text-muted small">Chýa có ch? ð? nào.</div>
                ) : (
